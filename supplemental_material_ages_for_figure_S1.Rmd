---
title: "The Natural History of Hypertension in Older Adults: A Study of Two Finnish Generational Cohorts Born 20 Years Apart - supplemental materials, ages"
output:
...
# Load the rest
p_load(ggplot2, ggthemes, dplyr, knitr, kableExtra, here, lubridate, tidyverse, janitor)
...
here::i_am("article1/table2_SBP.Rmd")
...
# Some of the BP data is in wrong format (or erroenous) - fix that
<br><br>
...
article1tuva$SBP_95_TUVA5_ist <- as.numeric(article1tuva$FYSMIT6b1a)
article1tuva$DBP_95_TUVA5_ist <- as.numeric(article1tuva$FYSMIT6b1b)

is.numeric(article1tuva$SBP_95_TUVA5_ist)
is.numeric(article1tuva$DBP_95_TUVA5_ist)

```


UTUVA3 has non-numeric BP values and one erroneously high SBP reading.


```{r fix_bp_data_utuva3, echo=TRUE}

# UTUVA3
article1utuva$RR_syst_istuen_U80 <- as.numeric(article1utuva$RR_syst_istuen_U80)

# There is one erroneous reading that is > 1000 mmHg, identify the row index
error_pos1 <- which(article1utuva$RR_syst_istuen_U80 > 400)

#Display the value
article1utuva$RR_syst_istuen_U80[error_pos1]

#Code as NA
article1utuva$RR_syst_istuen_U80[error_pos1] <- NA

#Confirm
is.numeric(article1utuva$RR_syst_istuen_U80)
min(article1utuva$RR_syst_istuen_U80, na.rm = T)
max(article1utuva$RR_syst_istuen_U80, na.rm = T)
...
tuva1_n <- sum(!is.na(article1tuva$SRR_70_istuen))
tuva1_n


# TUVA2
tuva2_n <- sum(!is.na(article1tuva$SRR_80_istuen))
tuva2_n


#TUVA3
tuva3_n <- sum(!is.na(article1tuva$SRR_istuen_85))
tuva3_n


#TUVA4
tuva4_n <- sum(!is.na(article1tuva$SRR_90_istuen))
tuva4_n


#TUVA5
tuva5_n <- sum(!is.na(article1tuva$SBP_95_TUVA5_ist))
tuva5_n


#UTUVA1
utuva1_n <- sum(!is.na(article1utuva$B8_3_A7isys_U70))
utuva1_n


#UTUVA2
utuva2_n <- sum(!is.na(article1utuva$B10b_A7_2_1_U75))
utuva2_n

 
#UTUVA3
utuva3_n <- sum(!is.na(article1utuva$RR_syst_istuen_U80))
utuva3_n
...
article1utuva <- article1utuva %>% mutate(utuva2date = paste0(B10b_c_U75,"-",
                                                              B10b_b_U75,"-",
                                                              B10b_a_U75))
...
# The mean SBPs of TUVA and UTUVA time points
<br><br>
...
tuva1_sbp <- mean(article1tuva$SRR_70_istuen, na.rm = TRUE)
tuva1_sbp
...
tuva2_sbp <- mean(article1tuva$SRR_80_istuen, na.rm = TRUE)
tuva2_sbp
...
tuva3_sbp <- mean(article1tuva$SRR_istuen_85, na.rm = TRUE)
tuva3_sbp
...
tuva4_sbp <- mean(article1tuva$SRR_90_istuen, na.rm = TRUE)
tuva4_sbp
...
tuva5_sbp <- mean(article1tuva$SBP_95_TUVA5_ist, na.rm = TRUE)
tuva5_sbp
...
utuva1_sbp <- mean(article1utuva$B8_3_A7isys_U70, na.rm = TRUE)
utuva1_sbp
...
utuva2_sbp <- mean(article1utuva$B10b_A7_2_1_U75, na.rm = TRUE)
utuva2_sbp
...
utuva3_sbp <- mean(article1utuva$RR_syst_istuen_U80, na.rm = TRUE)
utuva3_sbp
...
tuva1_age <-  article1tuva %>% drop_na(SRR_70_istuen) %>% select(IKA_tutkpvm_70) %>% unlist() %>% mean(., na.rm = T)
...
tuva2_age <-  article1tuva %>% drop_na(SRR_80_istuen) %>% select(tuva2age) %>% unlist() %>% mean(., na.rm = T)

tuva2_age

article1tuva %>% drop_na(SRR_80_istuen) %>% nrow()
...
tuva3_age <-  article1tuva %>% drop_na(SRR_istuen_85) %>% select(tuva3age) %>% unlist() %>% mean(., na.rm = T)

tuva3_age

article1tuva %>% drop_na(SRR_istuen_85) %>% nrow()
...
tuva4_age <-  article1tuva %>% drop_na(SRR_90_istuen) %>% select(tuva4age) %>% unlist() %>% mean(., na.rm = T)

tuva4_age


article1tuva %>% drop_na(SRR_90_istuen) %>% nrow()
...
tuva5_age <-  article1tuva %>% drop_na(SBP_95_TUVA5_ist) %>% select(tuva5age) %>% unlist() %>% mean(., na.rm = T)

tuva5_age

article1tuva %>% drop_na(SBP_95_TUVA5_ist) %>% nrow()
...
utuva1_age <-  article1utuva %>% drop_na(B8_3_A7isys_U70) %>% select(utuva1_age) %>% unlist() %>% mean(., na.rm = T)

utuva1_age

article1utuva %>% drop_na(B8_3_A7isys_U70) %>% nrow()
...
utuva2_age <-  article1utuva %>% drop_na(B10b_A7_2_1_U75) %>% select(utuva2_age) %>% unlist() %>% mean(., na.rm = T)

utuva2_age

article1utuva %>% drop_na(B10b_A7_2_1_U75) %>% nrow()
...
utuva3_age <-  article1utuva %>% drop_na(RR_syst_istuen_U80) %>% select(utuva3_age) %>% unlist() %>% mean(., na.rm = T)

utuva3_age



 article1utuva %>% drop_na(RR_syst_istuen_U80) %>% nrow()
...
# Run model
tuva1_vs_tuva2 <-t.test(article1tuva$SRR_70_istuen, article1tuva$SRR_80_istuen, paired=TRUE)
tuva1_vs_tuva2
...
# Run model
tuva1_vs_tuva3 <-t.test(article1tuva$SRR_70_istuen, article1tuva$SRR_istuen_85, paired=TRUE)
tuva1_vs_tuva3
...
# Run model
tuva1_vs_tuva4 <-t.test(article1tuva$SRR_70_istuen,  article1tuva$SRR_90_istuen, paired=TRUE)
tuva1_vs_tuva4
...
# Run model
tuva1_vs_tuva5 <-t.test(article1tuva$SRR_70_istuen, article1tuva$SBP_95_TUVA5_ist, paired=TRUE)
tuva1_vs_tuva5
...
# Run model
utuva1_vs_utuva2 <-t.test(article1utuva$B8_3_A7isys_U70, article1utuva$B10b_A7_2_1_U75, paired=TRUE)
...
# Run model
utuva1_vs_utuva3 <-t.test(article1utuva$B8_3_A7isys_U70, article1utuva$RR_syst_istuen_U80, paired=TRUE)
...
tuva1_utuva1_subtraction <- mean(article1tuva$SRR_70_istuen, na.rm = TRUE) - mean(article1utuva$B8_3_A7isys_U70, na.rm = TRUE)
...
# Run model
tuva1_vs_utuva1 <-t.test(article1tuva$SRR_70_istuen, article1utuva$B8_3_A7isys_U70, paired=FALSE)
...
tuva2_utuva3_subtraction <- mean(article1tuva$SRR_80_istuen, na.rm = TRUE) - mean(article1utuva$RR_syst_istuen_U80, na.rm = TRUE)
...
# Run model
tuva2_vs_utuva3 <-t.test(article1tuva$SRR_80_istuen, article1utuva$RR_syst_istuen_U80, paired=FALSE)
